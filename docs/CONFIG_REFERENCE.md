...
| `run_apply_limit` | int | `20` | Жёсткий лимит успешных откликов за один запуск аккаунта. Не заменяет дневной лимит; при ручном продолжении после его достижения начинается новый run. `0` = без ограничения. |
| `search_only_mode` | bool | `False` | Безопасный предпросмотр: сбор и фильтрация вакансий работают, очередь сохраняется, apply-фаза и сетевой вызов отправки блокируются |
...
| `letter_templates` | list | 1 шаблон «Стандартное» | `[{name, text}]` шаблоны сопроводительных писем; плейсхолдеры `[ИМЯ]`, `[contact details]`, `[📞 телефон]`. Выдаются в снапшот UI и в apply-путь (`manager:917`) |
