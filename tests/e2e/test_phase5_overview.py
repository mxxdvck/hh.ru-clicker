...
def test_phase5_overview_persisted_review_survives_empty_live_llm_log(ui):
    ...
    ui.data["interviews_summary"] = {
        "total": 3, "drafts": 2, "reviews": 2, "pending": 0, "replied": 1,
    }
    page = _open_overview(ui)
...
def test_phase5_overview_attention_links_cookies_to_connection_settings(ui):
    ...
    expect(page.get_by_test_id("phase5-settings-group-connection")).to_have_attribute("aria-pressed", "true")
...
def test_phase5_overview_review_summary_is_not_fetched_on_every_snapshot(ui):
    ...
    def summary_calls():
        return [call for call in ui.calls if call.get("method") == "GET" and call.get("path") == "/api/interviews/summary"]
...
def test_phase5_overview_account_health_has_runtime_details_and_pause_action(ui):
    ui.state["accounts"][0].update({
        "daily_limit": 20,
        "status": "applying",
        "status_detail": "Отправляю отклики",
        "current_vacancy_title": "1С разработчик",
        "current_vacancy_company": "Ромашка",
        "next_resume_touch": "12:30 (1ч20м)",
        "llm_enabled": True,
        "llm_pending_chats": 1,
    })
    page = _open_overview(ui)
    health = page.get_by_test_id("phase5-health-account-0")
    expect(health).to_contain_text("Фаза: applying")
    expect(health).to_contain_text("LLM: вкл")
    expect(health).to_contain_text("Сегодня: 7 / 20")

    health.locator("summary", has_text="Подробнее").click()
    expect(health).to_contain_text("1С разработчик · Ромашка")
    expect(health).to_contain_text("12:30 (1ч20м)")
    expect(health).to_contain_text("LLM очередь: 1")

    page.get_by_test_id("phase5-health-toggle-0").click()
    ui.wait_until(
        lambda: any(cmd.get("type") == "account_pause" and cmd.get("idx") == 0 for cmd in ui.commands),
        message="phase5 account_pause command",
    )


def test_phase5_overview_aggregates_per_account_limits_without_lying_in_header(ui):
    second = copy.deepcopy(ui.state["accounts"][0])
    second.update({"idx": 1, "name": "Мария Тестова", "short": "МТ", "daily_sent": 6, "sent": 4, "daily_limit": 20})
    ui.state["accounts"][0]["daily_limit"] = 20
    ui.state["accounts"].append(second)
    ui.state["config"]["daily_apply_limit"] = 20
    page = _open_overview(ui)

    expect(_kpi_value(page, "today")).to_have_text("13")
    expect(_kpi_value(page, "remaining")).to_have_text("27")
    expect(page.locator("#hdr-daily-counter")).to_have_text("(13/40 сегодня · 20/акк.)")


def test_phase5_overview_account_health_is_mobile_reachable(ui):
    ui.state["accounts"][0]["daily_limit"] = 20
    page = _open_overview(ui)
    health = page.get_by_test_id("phase5-health-account-0")
